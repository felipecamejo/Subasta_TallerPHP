...
export class RegistroGoogleComponent implements OnInit, AfterViewInit {
  ...
  ngOnInit(): void {
    ...
    this.form = this.fb.group({
      ...
      email: [email, [Validators.required, Validators.email]], // ahora es campo habilitado y editable
  ...
  enviar() {
    ...
    if (payload.rol === 'cliente' || payload.rol === 'rematador') {
      delete payload.idFiscal;
    }
    ...
    this.http.post(url, payload).subscribe({
      ...
      error: (err: HttpErrorResponse) => {
        if (err.status === 403 && err.error?.error?.includes('no fue aprobada')) {
          ...
          alert('Faltan campos requeridos. Verificá los datos.');
        } else {
